...
def download_youtube_audio_as_wav(youtube_url, output_dir='test'):
    ...
    try:
        ...
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ...
            # print(f"Fișier audio salvat la: {filename}")
            return filename
    ...
    for root, _, files in os.walk(dirr):
        for file in files:
            if file.endswith(".webm"):
                ...
                # Comanda FFmpeg pentru conversie (ascunde ieșirea FFmpeg)
                command = f'ffmpeg -loglevel quiet -i "{input_file}" -ab 192k -ac 2 "{output_file}"'
